...
filter on mechanically, so this mirrors ethical_screen.py's two-tier approach:
    1. NAMED-TICKER FALLBACK (KNOWN_ETF_TICKERS): the specific leveraged
       ...
       network call, and still catches them even if the yfinance lookup
       below is unavailable/rate-limited.
    2. MECHANICAL CHECK (yfinance quoteType == "ETF"): generalizes to any
       ticker never seen before, so a future volatility_screen.py refresh
       that picks up a new ETF doesn't require a manual list update to catch
       it — this runs for real (network available) in the daily GitHub
       Actions job, unlike ad-hoc local sandboxes that may block Yahoo.
...
def is_etf(ticker: str) -> bool:
    ...
    try:
        ...
        print(f"WARNING: quoteType lookup failed for {ticker}, keeping it ({exc})")
        return False
